 ...
 * @return int
 */
function solve_expression(string $exp): int
{
    for ($i = 0; $i < 10; $i++) {
        if (in_array($i, str_split($exp))) {
            continue;
        }

        preg_match('/(^-*[1-9][0-9]*|^0)(-|\+|\*)(-*[1-9][0-9]*|0)=(-*[1-9][0-9]*|0$)/', str_replace('?', $i, $exp), $result);

        if (empty($result)) {
            continue;
        }

        $left_exp = match ($result[2]) {
            '+' => $result[1] + $result[3],
            '-' => $result[1] - $result[3],
            '*' => $result[1] * $result[3],
        };

        if ($left_exp == $result[4]) {
            return $i;
        }
    }
